...
if (length(args) < 2) {
  stop("Usage: Rscript submission-tools/model_template.R YYYY-MM-DD team-model [input.csv] [output.csv]")
}
...
retrospective_input <- file.path(
  "retrospective-data", "2025-2026", "rounds",
  paste0(reference_date, ".csv")
)
default_input <- if (file.exists(retrospective_input)) {
  retrospective_input
} else {
  file.path("target-data", "time-series.csv")
}
input_path <- if (length(args) >= 3) args[[3]] else default_input
...
write.csv(submission, output_path, row.names = FALSE, na = "")
